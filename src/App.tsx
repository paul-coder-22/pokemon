...
class App extends React.Component<any,AppState> {
  ...
  handleInputChange = (inputValue : string) => {
    ...
    const searchedPokemons = allPokemons.filter((pokemon:PokemonSchema) => {
      return (pokemon.name && pokemon.name.toLowerCase().includes(inputValue.toLowerCase()))
    })
    this.setState({
  ...
  handleOnClick = (pokemonName : string) =>{
   const { allPokemons } = this.state;
   const selectedPokemons = allPokemons.find((pokemon:PokemonSchema) => pokemon.name === pokemonName)
  ...
  render(): React.ReactNode {
      return (
        <div className="App">
          <PokeDesk onInputChange={this.handleInputChange}
